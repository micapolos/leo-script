...
val Value.optionOrNull: Value?
  get() =
    resolvePrefixOrNull(optionName) { it }

val Value.presentOrNull: Value?
  get() =
    resolvePrefixOrNull(presentName) { it }

val Value.absentOrNull: Value?
  get() =
    resolvePrefixOrNull(absentName) { it }

val Value.presentOption: Value
  get() =
    make(presentName).make(optionName)

val absentOptionValue: Value
  get() =
    value().make(absentName).make(optionName)

val Value.liftToOption: Value
  get() =

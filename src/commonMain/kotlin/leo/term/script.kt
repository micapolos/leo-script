...
val <T> Term<T>.scriptLine: ScriptLine get() =
	"term" lineTo script

val <T> Term<T>.script: Script get() =
	when (this) {
...
val TermVariable.script: Script get() =
	script("variable" lineTo script(literal(index)))

val <T> T.nativeScript: Script get() =
	script("native" lineTo script(literal(toString())))

val <T> TermApplication<T>.script: Script get() =

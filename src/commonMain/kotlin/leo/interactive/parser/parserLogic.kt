...
val Char.digitOrNull: Digit?
  get() = try {
    digitToInt().digitOrNull
  } catch (e: Exception) {
    null
  }
...
val Char.numberPrefixOrNull: NumberPrefix?
  get() =
    null
...
val NumberNegative.literalOrNull: Literal?
  get() =
...
val NumberPrefix.literalOrNull: Literal?
  get() =
    when (this) {
...
val Char.textOpeningOrNull: TextOpening?
  get() =
...
val CharEscaped.unescapedChar: Char
  get() =
    when (char) {
      'n' -> '\n'
      '\\' -> '\\'
...
val TextOpening.stringOrNull: String?
  get() =
    notNullIf(itemPrefixOrNull == null) {
      itemStack.map { char }.charString
    }

val Char.textItemOrNull: TextItem?
  get() =
    when (this) {
      '\\' -> null
      '\n' -> null
...
val TextPrefix.literalOrNull: Literal?
  get() =
    when (this) {
...
val AtomPrefix.literalPrefixOrNull: LiteralPrefix?
  get() =
    (this as? LiteralAtomPrefix)?.literalPrefix

val AtomPrefix.wordOrNull: Word?
  get() =
    (this as? WordAtomPrefix)?.word

val LiteralPrefix.literalOrNull: Literal?
  get() =
    when (this) {
      is NumberLiteralPrefix -> numberPrefix.literalOrNull
...
val LiteralPrefix.atomOrNull: Atom?
  get() =
    literalOrNull?.let(::atom)

val AtomPrefix.atomOrNull: Atom?
  get() =
    when (this) {
...
val Tab.tokens: Tokens
  get() =
    stack(endStackLink).map { token(this) }.let(::Tokens)

val IndentSuffix.tokens: Tokens
  get() =
...
val Spaceable.atomPrefixOrNull: AtomPrefix?
  get() =
    when (this) {
...
val Spaced.atomPrefixOrNull: AtomPrefix?
  get() =
    spaceable.atomPrefixOrNull

val Spaceable.clearAtomPrefix: Spaceable
  get() =
    when (this) {
...
val Spaced.clearAtomPrefix: Spaced
  get() =
    spaced(spaceable.clearAtomPrefix)

val Spaceable.tabOrNull: Tab?
  get() =
    when (this) {
...
val Spaced.tabOrNull: Tab?
  get() =
    spaceable.tabOrNull?.plus(end) ?: tab(end)

val Spaceable.beginTab: Tab
  get() =
    when (this) {
...
val Spaced.beginTab: Tab
  get() =
    spaceable.beginTab.plus(end)

val Spaceable.tabTokens: Tokens
  get() =
                  ...
                  is LiteralAtom ->
                    prefix(
                      ...
                      line(
                        header(
                          prefix(indent(), null),
                          suffix(body.indent.ifNotNull(body.commable.spaceable.tabOrNull) { plus(it) }))))
                  is NameAtom ->
                    prefix(
...
val Header.endTokensOrNull: Tokens?
  get() =
    notNullIf(prefix.isEmpty) { suffix.tokens }

val Line.endTokensOrNull: Tokens?
  get() =
    when (this) {
      is BodyLine -> null
...
val TokensPrefix.endTokensOrNull: Tokens?
  get() =

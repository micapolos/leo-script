...
val Value.resolveOrNull: Value?
  get() =
    null
      ?: onlyFieldOrNull?.resolveValueOrNull

val Field.resolveValueOrNull: Value?
  get() =
    null
      ?: resolveGetOrNull
      ?: resolveCommandOrNull

val Field.resolveGetOrNull: Value?
  get() =
    rhs.valueOrNull?.getOrNull(name)

val Field.resolveCommandOrNull: Value?
  get() =
    when (name) {
...
val Value.doubleResolveOrNull: Double?
  get() =
    if (isEmpty) null
...
val Field.digitOrNull: Int?
  get() =
    rhsOrNull(name)?.valueOrNull?.nameOrNull?.digitOrNull

val String.digitOrNull: Int?
  get() =
    when (this) {
      "zero" -> 0
      "one" -> 1

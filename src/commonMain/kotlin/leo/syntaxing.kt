...
val Script.syntaxSyntaxing: Syntaxing<Syntax>
  get() =
    syntax().syntaxing.foldStateful(lineSeq.reverse) { plusCompilation(it) }

val ScriptLine.syntaxLineSyntaxing: Syntaxing<SyntaxLine>
  get() =
    null
      ?: specialNumberScriptLineOrNull?.syntaxing
      ?: defaultSyntaxLineSyntaxing

val ScriptLine.defaultSyntaxLineSyntaxing: Syntaxing<SyntaxLine>
  get() =
    when (this) {
...
val ScriptLine.syntaxAtomSyntaxing: Syntaxing<SyntaxAtom>
  get() =
    when (this) {
...
val ScriptLine.syntaxFieldSyntaxing: Syntaxing<SyntaxField>
  get() =
    fieldOrNull
...
val ScriptField.syntaxLineSyntaxing: Syntaxing<SyntaxLine>
  get() =
    when (name) {
...
val ScriptField.syntaxAtomSyntaxing: Syntaxing<SyntaxAtom>
  get() =
    syntaxFieldSyntaxing.map(::atom)

val ScriptField.syntaxFieldSyntaxing: Syntaxing<SyntaxField>
  get() =
    rhs.syntaxSyntaxing.map { rhsSyntax ->
      name fieldTo rhsSyntax
    }

val ScriptLine.specialNumberScriptLineOrNull: SyntaxLine?
  get() =
    when (this) {
...
val Literal.syntaxLineSyntaxing: Syntaxing<SyntaxAtom>
  get() =
    syntaxAtom(this).syntaxing

val Script.selectSyntaxing: Syntaxing<Select>
  get() =
    lineStack.map { caseSyntaxing }.flat.map(::Select)

val ScriptLine.caseSyntaxing: Syntaxing<Case>
  get() =
    when (this) {
...
val ScriptField.caseSyntaxing: Syntaxing<Case>
  get() =
    rhs.syntaxSyntaxing.map { syntax ->
      name caseTo syntax
    }

val Script.asSyntaxing: Syntaxing<As>
  get() =
    syntaxSyntaxing.map(::as_)

val Script.beSyntaxing: Syntaxing<Be>
  get() =
    syntaxSyntaxing.map(::be)

val Script.beingSyntaxingOrNull: Syntaxing<Being>?
  get() =
    notNullIf(!isEmpty) {
      syntaxSyntaxing.map(::being)
    }

val Script.applySyntaxing: Syntaxing<Apply>
  get() =
    blockSyntaxing.map(::apply)

val Script.applyingSyntaxingOrNull: Syntaxing<Applying>?
  get() =
    notNullIf(!isEmpty && !equals(script(anythingName))) {
      blockSyntaxing.map(::applying)
    }

val Script.endSyntaxing: Syntaxing<End>
  get() =
    syntaxSyntaxing.map(::end)

val Script.blockSyntaxing: Syntaxing<Block>
  get() =
    null
      ?: recursingSyntaxingOrNull?.map(::block)
      ?: syntaxSyntaxing.map(::block)

val Script.recursingSyntaxingOrNull: Syntaxing<Recursing>?
  get() =
    matchPrefix(recursingName) { rhs ->
      rhs.syntaxSyntaxing.map(::recursing)
    }

val Script.checkSyntaxing: Syntaxing<Check>
  get() =
    isSyntaxing.map(::check)

val Script.combineWithSyntaxingOrNull: Syntaxing<CombineWith>?
  get() =
    matchPrefix(withName) { withScript ->
      withScript.blockSyntaxing.map(::combineWith)
    }

val Script.combiningWithSyntaxingOrNull: Syntaxing<CombiningWith>?
  get() =
    matchPrefix(withName) { withScript ->
...
val Script.commentSyntaxing: Syntaxing<Comment>
  get() =
    comment(this).syntaxing

val Script.debugSyntaxing: Syntaxing<Debug>
  get() =
    debug(this).syntaxing

val Script.doSyntaxing: Syntaxing<Do>
  get() =
    blockSyntaxing.map(::do_)

val Script.haveSyntaxing: Syntaxing<Have>
  get() =
    syntaxSyntaxing.map(::have)

val Script.doingSyntaxingOrNull: Syntaxing<Doing>?
  get() =
    notNullIf(!isEmpty && !equals(script(anythingName))) {
      blockSyntaxing.map(::doing)
    }

val Script.exampleSyntaxing: Syntaxing<Example>
  get() =
    syntaxSyntaxing.map(::example)

val Script.getSyntaxing: Syntaxing<Get>
  get() =
    lineStack
...
val Script.giveSyntaxing: Syntaxing<Give>
  get() =
    syntaxSyntaxing.map(::give)

val Script.helpSyntaxing: Syntaxing<Help>
  get() =
    help(this).syntaxing

val Script.letSyntaxing: Syntaxing<Let>
  get() =
...
val Script.repeatSyntaxing: Syntaxing<Repeat>
  get() =
    syntaxSyntaxing.map(::repeat)

val Script.privateSyntaxing: Syntaxing<Private>
  get() =
    syntaxSyntaxing.map(::private)

val Script.failSyntaxing: Syntaxing<Fail>
  get() =
    syntaxSyntaxing.map(::fail)

val Script.isSyntaxing: Syntaxing<Is>
  get() =
    matchPrefix(notName) { rhs ->
      rhs.isRhsSyntaxing.map(::is_).map { it.negate }
    } ?: isRhsSyntaxing.map(::is_)

val Script.isRhsSyntaxing: Syntaxing<IsRhs>
  get() =
...
val Script.loadSyntaxing: Syntaxing<Load>
  get() =
    loadOrNull.notNullOrThrow { value(loadName fieldTo value) }.syntaxing

val Script.matchingSyntaxing: Syntaxing<Matching>
  get() =
    syntaxSyntaxing.map(::matching)

val Script.equalSyntaxing: Syntaxing<Equal>?
  get() =
    matchPrefix(toName) { rhs ->
      rhs.syntaxSyntaxing.map(::equal)
    }

val Script.recurseSyntaxing: Syntaxing<Recurse>
  get() =
    syntaxSyntaxing.map(::recurse)

val Script.recursiveSyntaxing: Syntaxing<Recursive>
  get() =
    syntaxSyntaxing.map(::recursive)

val Script.quoteSyntaxing: Syntaxing<Quote>
  get() =
    quote(this).syntaxing

val Script.setSyntaxing: Syntaxing<Set>
  get() =
    lineStack.map { syntaxAtomSyntaxing }.flat.map(::Set)

val Script.takeSyntaxing: Syntaxing<Take>
  get() =
    syntaxSyntaxing.map(::take)

val Script.testSyntaxing: Syntaxing<Test>
  get() =
...
val Script.theSyntaxing: Syntaxing<The>
  get() =
    lineStack.map { syntaxAtomSyntaxing }.flat.map(::the)

val Script.trySyntaxing: Syntaxing<Try>
  get() =
    syntaxSyntaxing.map(::try_)

val Script.updateSyntaxing: Syntaxing<Update>
  get() =
    lineStack.map { syntaxFieldSyntaxing }.flat.map(::Update)

val Script.useSyntaxing: Syntaxing<Use>
  get() =
    useOrNull.notNullOrThrow { value(useName fieldTo value) }.syntaxing

val Script.withSyntaxing: Syntaxing<With>
  get() =

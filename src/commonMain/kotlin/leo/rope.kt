...
val <T> Stack<T>.ropeOrNull: Rope<T>?
  get() =
    linkOrNull?.rope

val <T> StackLink<T>.rope: Rope<T>
  get() =
    Rope(tail, head, stack())

val <T> Rope<T>.backOrNull: Rope<T>?
  get() =
    when (tail) {
      is EmptyStack -> null
...
val <T> Rope<T>.forwardOrNull: Rope<T>?
  get() =
    when (head) {
      is EmptyStack -> null
...
val <T> Rope<T>.stackLink: StackLink<T>
  get() =
    (tail linkTo current).fold(head) { push(it) }

val <T> Rope<T>.stack: Stack<T>
  get() =
    stackLink.stack

val <T> Rope<T>?.orNullStack: Stack<T>
  get() =
